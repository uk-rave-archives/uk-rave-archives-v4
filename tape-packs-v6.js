(() => {
  ...
  const selectedYearTitle = document.querySelector('#selected-year-title');
  const selectedYearSummary = document.querySelector('#selected-year-summary');
  ...
  const ARCHIVE_YEARS = Array.from({length: 18}, (_, index) => 1988 + index);
  ...
  function yearCount(year) {
    return packs.filter(pack => pack.year === year).length;
  }

  function availableYears() {
    return ARCHIVE_YEARS.filter(year => yearCount(year) > 0);
  }

  function fillYears() {
    const yearsWithPacks = availableYears();
    if (!selectedYear || !yearsWithPacks.includes(selectedYear)) selectedYear = yearsWithPacks[0] || 1988;
    yearTabs.innerHTML = ARCHIVE_YEARS.map(year => {
      const count = yearCount(year);
      return `<button type="button" class="year-tab${year === selectedYear ? ' active' : ''}${count ? '' : ' empty'}" data-year="${year}" ${count ? '' : 'disabled'}><strong>${year}</strong><span>${count || '—'}</span></button>`;
    }).join('');
  }
  ...
  function updateYearControls() {
    ...
    previousYear.disabled = index <= 0;
    nextYear.disabled = index < 0 || index >= years.length - 1;
    yearTabs.querySelectorAll('.year-tab').forEach(button => button.classList.toggle('active', Number(button.dataset.year) === selectedYear));
  }
  ...
  function render() {
    ...
    const filtered = packs.filter(pack => {
      ...
      return (isSearching || pack.year === selectedYear) && (!q || hay.includes(q));
    });

    if (isSearching) {
      const grouped = new Map();
      filtered.forEach(pack => {
        const key = pack.year || 'Undated';
        if (!grouped.has(key)) grouped.set(key, []);
        grouped.get(key).push(pack);
      });
      catalogue.innerHTML = [...grouped.entries()].sort((a,b) => String(a[0]).localeCompare(String(b[0]), undefined, {numeric:true})).map(([year, items]) => `
        <section class="year-result-group">
          <header><h3>${esc(year)}</h3><span>${items.length} pack${items.length === 1 ? '' : 's'}</span></header>
          <div class="year-card-grid">${items.map(packCard).join('')}</div>
        </section>`).join('');
      selectedYearTitle.textContent = 'Search results';
      selectedYearSummary.textContent = `Searching every mapped year for “${search.value.trim()}”.`;
      resultsMessage.textContent = `${filtered.length} matching event pack${filtered.length === 1 ? '' : 's'} across the archive`;
    } else {
      catalogue.innerHTML = filtered.map(packCard).join('');
      selectedYearTitle.textContent = String(selectedYear);
      const recordings = filtered.reduce((total, pack) => total + pack.tracks.length, 0);
      selectedYearSummary.textContent = filtered.length
        ? `${filtered.length} event pack${filtered.length === 1 ? '' : 's'} · ${recordings} mapped recording${recordings === 1 ? '' : 's'}`
        : 'No verified tape packs have been mapped for this year yet.';
      resultsMessage.textContent = filtered.length ? `Showing the ${selectedYear} gallery only` : `The ${selectedYear} gallery is awaiting verified material`;
    }
    updateYearControls();
  }
  ...
  clearFilters.addEventListener('click',()=>{search.value='';render();});
  yearTabs.addEventListener('click', event => {
    const button = event.target.closest('.year-tab:not(:disabled)');
    if (!button) return;
  ...
  previousYear.addEventListener('click', () => {
    const years = availableYears();
    const index = years.indexOf(selectedYear);
    if (index > 0) { selectedYear = years[index - 1]; search.value=''; render(); }
  });
  nextYear.addEventListener('click', () => {
    const years = availableYears();
    const index = years.indexOf(selectedYear);
    if (index >= 0 && index < years.length - 1) { selectedYear = years[index + 1]; search.value=''; render(); }
  });
